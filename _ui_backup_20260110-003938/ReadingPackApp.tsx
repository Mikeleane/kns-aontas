    ...
    <div class="pb">
      <div class="labGrid">
        <div class="card">
          ...
          <div class="tiny">Select text â†’ "Add to Vocab Lab". Then practise here.</div>
...
(function(){
  ...
  function bionicify(text){
    return text.split(/(\s+)/).map(function(tok){
      ...
      var w = tok;
      var m = w.match(/^([\(\[\{{\"'"""˜']*)([A-Za-zÃ€-Ã–Ã˜-Ã¶Ã¸-Ã¿-]+)([\)\]\}}\.,;:!?'""""˜']*)$/);
  ...
  function renderVocab(){
    ...
    if(!state.vocab.length){
      ...
      d.className = "tiny";
      d.textContent = "No saved words yet. Select a word/phrase in the text and add it "” or press "Suggest words".";
  ...
  function scoreWord(stage, w){
    ...
    // Simple "interestingness" score that adapts by Stage.
...
function makeSupportBoxDocx(args: { adapted?: boolean }) {
  const adapted = args.adapted === true;

  const baseSize = adapted ? 26 : 24;
  const smallSize = adapted ? 24 : 22;
  const line = adapted ? 420 : 360;
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: `${args.title} "¢ ${classLabel(args.schoolClass)} "¢ ${stageLabel(args.stage)} "¢ Student Sheet (${adapted ? "B" : "A"})`,
                bold: true,
                size: adapted ? 22 : 20,
                font: "Calibri",
              }),
          ...
          ...(adapted ? [makeSupportBoxDocx({ adapted }), new Paragraph({ text: "" })] : []),
...
function buildReadingStudentPdf(args: {
  ...
  // PDF safety: jsPDF built-in fonts are limited; strip common "smart" punctuation.
  const pdfSafe = (t: any) =>
    String(t ?? "")
      .replace(/["“"”]/g, "-")
      .replace(/["˜']/g, "'")
  ...
  doc.text(pdfSafe(`${args.title} - ${classLabel(args.schoolClass)} - ${stageLabel(args.stage)} - Student Sheet (${args.mode === "adapted" ? "B" : "A"})`), margin, y);
  y += adapted ? 7 : 6;
...
return (
    <main className="min-h-screen kns-bg text-slate-100">
  ...
  <div className="flex items-center gap-3">
    ...
    <div className="flex flex-col">
      ...
      <p className="text-slate-400 text-xs">Paste text, fetch an article, or (coming soon) drop in a photo/screenshot of a page.</p>
    </div>
  </div>
</header>
