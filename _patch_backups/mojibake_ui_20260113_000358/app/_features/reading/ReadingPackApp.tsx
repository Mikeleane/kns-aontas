...
/**
 * Aontas 10 â€” unified generator page
...
const QUESTION_FOCUS_OPTIONS: Array<{ id: QuestionFocus; label: string; hint: string }> = [
  {
    ...
    hint: "Mix of gist, detail and some vocabulary â€” a good all-round reading lesson.",
  },
  {
    id: "whowhatwhere",
...
function buildReadingInteractiveHtml(args: {
  ...
  // - Select text â†’ quick tools: pronounce, add to vocab, lookup, translate, images
  // - Vocab & Pronunciation Lab: word bank + small â€œlisten & chooseÃ¢â‚¬Â retrieval game
  return String.raw`<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Aontas â€” Kilgobnet N.S. â€” Reading Pack</title>
<style>
  :root{
    --bg:#07101f;--panel:#0f1b33;--text:#e7eefc;--muted:#b1c1e6;--line:rgba(255,255,255,.12);
...
<div class="pop" id="pop">
  ...
  <div class="actions">
    <button class="linkbtn" id="popPronounce">Ã°Å¸â€Å  Pronounce</button>
    <button class="linkbtn" id="popAdd">Ã¢Å¾â€¢ Add to Vocab Lab</button>
    <button class="linkbtn" id="popDefine">Ã°Å¸â€œËœ Look up</button>
    <button class="linkbtn" id="popTranslate">Ã°Å¸Å’Â Translate</button>
    <button class="linkbtn" id="popImages">Ã°Å¸â€“Â¼Ã¯Â¸Â Images</button>
    <button class="linkbtn" id="popClose">Ã¢Å“â€“</button>
  </div>
...
<div class="wrap">
  <div class="top">
    ...
    <div class="row">
      ...
      <button class="btn" id="toggleText">Ã°Å¸â„¢Ë† Hide text</button>
    </div>
  </div>
  ...
  <section class="panel">
    <div class="ph">
      ...
      <div class="row">
        <button class="btn" id="readAll">Ã°Å¸â€Å  Read all</button>
        <button class="btn" id="pause">Ã¢ÂÂ¸ Pause</button>
        <button class="btn" id="stop">Ã¢ÂÂ¹ Stop</button>
    ...
    <div class="pb">
      <div class="row" style="margin-bottom:10px;">
        <button class="btn" id="fsDown">AÃ¢Ë†â€™</button>
        ...
        <button class="btn" id="rulerToggle">Ã°Å¸Å½Â¯ Focus ruler</button>
        <button class="btn" id="bionicToggle">Ã°Å¸Â§Â  Bionic</button>
        <button class="btn" id="chunkToggle">Ã°Å¸Â§Â© One at a time</button>
        <button class="btn" id="chunkPrev" style="display:none;">Ã¢â€”â‚¬ Prev</button>
        <button class="btn" id="chunkNext" style="display:none;">Next Ã¢â€“Â¶</button>
      ...
      <div class="tiny" style="margin-top:10px;">
        Read-aloud uses your browserâ€™s speech engine. If voices are missing, try reloading the page.
      </div>
    </div>
  </section>
    ...
    <div class="ph">
      <div>
        ...
        <div id="teacherPanel" class="row" style="display:none; margin-top:10px;">
          ...
          <button class="btn" id="teacherAnswers" type="button">Ã¢Å“â€¦ Answers</button>
    ...
    <div class="pb">
      <div class="labGrid">
        <div class="card">
          ...
          <div class="tiny">Select text â†’ â€œAdd to Vocab LabÃ¢â‚¬Â. Then practise here.</div>
          <div class="row" style="margin-top:10px;">
            <button class="btn" id="suggestVocab" type="button">Ã¢Å“Â¨ Suggest vocab</button>
          </div>
        ...
        <div class="card">
          ...
          <div class="tiny" id="practiceHint">Pick a word from the list, then press â€œPronounceÃ¢â‚¬Â.</div>
          <div style="margin-top:10px;" class="row">
            <button class="btn" id="sayWord">Ã°Å¸â€Å  Pronounce</button>
            <button class="btn" id="saySlow">Ã°Å¸ÂÂ¢ Slow</button>
            <button class="btn" id="example">Ã°Å¸â€œÅ’ Example</button>
          </div>
          ...
          <div class="row" style="margin-top:10px;">
            <button class="btn" id="startGame">Ã¢â€“Â¶ Start</button>
            <button class="btn" id="repeatGame">Ã°Å¸â€Â Repeat</button>
          </div>
...
(function(){
  ...
  function refreshTeacherUi(){
    ...
    if(teacherAnswers) teacherAnswers.textContent = (state.teacher.showAnswers ? "Ã¢Å“â€¦ Answers: ON" : "Ã¢Å“â€¦ Answers");
  ...
  subEl.textContent = (data.schoolClass ? ("Class " + data.schoolClass + " â€¢ Stage " + data.stage) : ("Stage " + (data.stage || "")));
  ...
  function bionicify(text){
    return text.split(/(\s+)/).map(function(tok){
      ...
      var w = tok;
      var m = w.match(/^([\(\[\{{\"'â€œÃ¢â‚¬Ââ€˜â€™]*)([A-Za-zÃƒâ‚¬-Ãƒâ€“ÃƒËœ-ÃƒÂ¶ÃƒÂ¸-ÃƒÂ¿-]+)([\)\]\}}\.,;:!?'"â€œÃ¢â‚¬Ââ€˜â€™]*)$/);
  ...
  function renderReading(){
    ...
    if(state.teacher && state.teacher.enabled && state.teacher.showLabels){
      subEl.textContent = (data.schoolClass ? ("Class " + data.schoolClass + " â€¢ Stage " + data.stage) : ("Stage " + (data.stage || ""))) + " â€¢ Classic = Standard â€¢ Spacious = Supported";
    } else {
  ...
  // Shared answer-key support:
  // We store answers as STANDARD option text (or a shared index). For ADAPTED mode,
  // the option text changes, but the *index* of the correct option stays the same.
  function getCorrectIndex(item){
    try{
      if(item && item.answerIndex != null) return Number(item.answerIndex);
      var std = item && item.standard ? item.standard : null;
      var stdOpts = (std && std.options) ? std.options : [];
      var ans = item ? item.answer : null;
  ...
  function getDisplayAnswer(item, side){
    try{
      var ans = item ? item.answer : null;
      if(Array.isArray(ans)) return ans.join("; ");
      var idx = getCorrectIndex(item);
  ...
  function renderQs(){
    ...
    exercises.forEach(function(item, idx){
      ...
      if(opts.length){
        opts.forEach(function(opt){
          ...
          o.addEventListener("click", function(){
            ...
            // Prefer shared-index checking (works across Standard/Adapted text)
            var correctIdx = getCorrectIndex(item);
            var selIdx = (opts || []).map(norm).indexOf(norm(opt));
            if(correctIdx >= 0 && selIdx >= 0) ok = (selIdx === correctIdx);
            else if(Array.isArray(item.answer)) ok = item.answer.indexOf(opt) !== -1;
            else ok = norm(item.answer) === norm(opt);

            fb.textContent = ok ? "Correct âœ…" : "Not yet â€” try again.";
      ...
      var ans = item.answer;
      var ansText = getDisplayAnswer(item, side);
  ...
  bionicToggle.addEventListener("click", function(){
    ...
    bionicToggle.textContent = state.bionic ? "Ã°Å¸Â§Â  Bionic: ON" : "Ã°Å¸Â§Â  Bionic";
    renderReading();
  });
  function applyChunkUi(){
    ...
    if(chunkToggle) chunkToggle.textContent = on ? "Ã°Å¸Â§Â© One at a time: ON" : "Ã°Å¸Â§Â© One at a time";
  }
  ...
  function setRuler(on){
    ...
    rulerToggle.textContent = on ? "Ã°Å¸Å½Â¯ Focus ruler: ON" : "Ã°Å¸Å½Â¯ Focus ruler";
  }
  ...
  function renderVocab(){
    ...
    if(!state.vocab.length){
      ...
      d.className = "tiny";
      d.textContent = "No saved words yet. Select a word/phrase in the text and add it â€” or press â€œSuggest wordsÃ¢â‚¬Â.";
      wordList.appendChild(d);
      practiceHint.textContent = "Pick an item from the list, then press â€œPronounceÃ¢â‚¬Â.";
      return;
    }
  ...
  function startListenGame(){
    ...
    choices.forEach(function(w){
      ...
      b.addEventListener("click", function(){
        ...
        else gameMsg.textContent = "Not yet â€” repeat and try again.";
      });
      right.appendChild(b);
  ...
  function tokenizeText(text){
    ...
    return t.match(/[a-zÃƒÂ -ÃƒÂ¶ÃƒÂ¸-ÃƒÂ¿][a-zÃƒÂ -ÃƒÂ¶ÃƒÂ¸-ÃƒÂ¿'\-]*/g) || [];
  }
  ...
  function scoreWord(stage, w){
    ...
    // Simple â€œinterestingnessÃ¢â‚¬Â score that adapts by Stage.
  ...
  // Tabs (UI only â€” both live together)
  ...
  toggleText.addEventListener("click", function(){
    ...
    toggleText.textContent = state.hide ? "Ã°Å¸â€˜â‚¬ Show text" : "Ã°Å¸â„¢Ë† Hide text";
    renderReading();
  });
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: `Aontas â€” ${KNS_SCHOOL_NAME} â€” Reading Pack`,
                bold: true,
                size: headingSize,
                font: "Calibri",
              }),
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: `${args.title} â€¢ ${classLabel(args.schoolClass)} â€¢ ${stageLabel(args.stage)} â€¢ Student Sheet (${SUPPORTED ? "B" : "A"})`,
                bold: true,
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            spacing: { after: 140 },
            children: [new TextRun({ text: `Aontas â€” ${KNS_SCHOOL_NAME} â€” Reading Pack`, bold: true, size: 30, font: "Calibri" })],
          }),
          new Paragraph({
            spacing: { after: 200 },
            children: [new TextRun({ text: `${args.title} â€¢ ${classLabel(args.schoolClass)} â€¢ ${stageLabel(args.stage)} â€¢ Teacher Key`, bold: true, size: 22, font: "Calibri" })],
          }),
...
function buildReadingStudentPdf(args: {
  ...
  // PDF safety: jsPDF built-in fonts are limited; strip common â€œsmartÃ¢â‚¬Â punctuation.
  const pdfSafe = (t: any) =>
    String(t ?? "")
      .replace(/[Ã¢â‚¬â€œâ€”]/g, "-")
      .replace(/[â€˜â€™]/g, "'")
...
function buildReadingTeacherKeyPdf(args: {
  ...
  doc.setFontSize(16);
  doc.text("Teacher Key â€” Reading Pack", margin, y);
  y += 7;

  doc.setFontSize(11);
  doc.text(`${args.title} â€¢ ${classLabel(args.schoolClass)} â€¢ ${stageLabel(args.stage)}`, margin, y);
  y += 6;
...
const stage = stageMode === "auto" ? stageAuto : stageManual;
  ...
  async function fetchArticle(e: FormEvent) {
    e.preventDefault();
    setReadingStatus("Fetching articleâ€¦");
    try {
  ...
  async function onPickImage(file?: File | null) {
    if (!file) return;
    setReadingStatus("Preparing imageâ€¦");
    try {
      const dataUrl = await fileToJpegDataUrl(file);
      setImageDataUrl(dataUrl);
      setImageName(file.name || "photo.jpg");
      setReadingStatus("Image ready. Click â€˜Extract textâ€™.");
    } catch (err: any) {
  ...
  async function extractTextFromImage() {
    ...
    setReadingStatus("Extracting text from imageâ€¦");
    try {
  ...
  async function generateReading() {
    setReadingStatus("Generating Standard/SUPPORTED readingâ€¦");
    try {
  ...
  async function generateExercises() {
    ...
    setReadingStatus("Generating exercisesâ€¦");
    try {
  ...
  async function exportTeacherPackZip() {
    ...
    setReadingStatus("Building zipâ€¦");
...
return (
  ...
  <div className="flex items-center gap-3">
    ...
    <div className="flex flex-col">
      <h1 className="text-2xl font-black tracking-tight">Aontas â€” Kilgobnet N.S.</h1>
      <p className="text-slate-600 text-sm">Inclusive reading packs (Standard + Supported) for one class, two streams.</p>
      <p className="text-slate-500 text-xs">Paste text, fetch an article, or drop in a photo/screenshot of a page.</p>
    </div>
  </div>
</header>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3">
          ...
          <span className="ml-auto text-sm text-slate-600">{readingStatus || "Ready."}</span>
        </div>
        {/* Source */}
    ...
    {STAGES.map((s) => (
      <option key={s} value={String(s)}>
        {stageLabel(s)}
              ...
              <label className="flex flex-col gap-1">
                <span className="text-xs text-slate-600">Output language</span>
                <select
              ...
              <label className="flex flex-col gap-1">
                <span className="text-xs text-slate-600">Text type</span>
                <select
                  value={textType}
                  onChange={(e) =>
              ...
              <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
                ...
                />
                <span className="text-sm text-slate-700">Dyslexia-friendly</span>
              </label>
            </div>

            <form onSubmit={fetchArticle} className="flex flex-col gap-2">
              <label className="text-xs text-slate-600">Article URL (optional)</label>
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 font-semibold hover:bg-slate-100"
                >
                  Fetch
                </button>
              </div>
              {!!articleTitle && (
                <div className="text-sm text-slate-600">Loaded: {articleTitle}</div>
              )}
            </form>

            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="text-sm font-extrabold">Photo / screenshot (optional)</div>
              <div className="mt-1 text-xs text-slate-500">Upload a photo/screenshot of a page and extract the text into the box below.</div>
              <div className="mt-3 flex flex-col gap-2">
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => onPickImage(e.target.files?.[0])}
                  className="block w-full text-sm text-slate-700 file:mr-3 file:rounded-lg file:border-0 file:bg-slate-50 file:px-3 file:py-2 file:font-semibold file:text-slate-900 hover:file:bg-white/15"
                />
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 font-semibold hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    Extract text
                  </button>
                  {imageName && <span className="text-xs text-slate-500">{imageName}</span>}
                </div>
            ...
            <label className="flex flex-col gap-1">
              <span className="text-xs text-slate-600">Input text</span>
              <textarea
                value={inputText}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="ml-auto text-sm text-slate-600">{readingStatus}</div>
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <div>
                  <div className="text-xs font-bold text-slate-600">Question focus</div>
                  <select
                    value={questionFocus}
                ...
                <div>
                  <div className="text-xs font-bold text-slate-600">Blocks</div>
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    {EXERCISE_BLOCKS.map((b) => {
                      ...
                      return (
                        <label
                          key={b.id}
                          className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white/40 px-3 py-2 text-sm hover:bg-white/60"
                        >
                          <input
                            type="checkbox"
                            checked={checked}
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-col gap-3">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-3">
                <div className="text-xs font-bold text-slate-600">SUPPORTED</div>
                <pre className="mt-2 whitespace-pre-wrap text-sm text-slate-900">
                  {SUPPORTEDReading || "â€”"}
                </pre>
              </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-3">
              <div className="text-xs font-bold text-slate-600">Exercises preview</div>
                  ...
                  exercises.slice(0, 6).map((q) => (
                    <div key={q.id} className="rounded-xl border border-slate-200 bg-slate-50 p-2">
                      ...
                      <div className="text-slate-600 text-xs">
                        Type: {q.type} â€¢ Skill: {q.skill}
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="text-slate-500">No exercises yet.</div>
                )}
              </div>
            </div>
          </div>
