# Apply-WordinessPolishEverywhere.ps1 (touch/tablet polish v3, safe + ASCII)
...
$cssMarker = "wordiness-ui-polish-v3"
$jsMarker  = "wordiness-touch-helpers-v3"
...
foreach ($f in $files) {
  ...
  # CSS (avoid duplicates: if any earlier v exists, skip)
  if ($src -notmatch '(?i)wordiness-ui-polish-v') {
    if ($src -match '(?i)</head>') { $src = Replace-First $src '(?i)</head>' ($cssBlock + "`r`n</head>") }
    else { $src += "`r`n" + $cssBlock }
  }

  # JS (avoid duplicates)
  if ($src -notmatch '(?i)wordiness-touch-helpers-v') {
    if ($src -match '(?i)</body>') { $src = Replace-First $src '(?i)</body>' ($jsBlock + "`r`n</body>") }
    else { $src += "`r`n" + $jsBlock }
  }
