# Add-WordinessNextTinyStepBuilder.ps1
...
function Escape-NewlinesInsideJsonStrings([string]$s) {
  # Converts raw CR/LF occurring inside JSON strings into \n so ConvertFrom-Json can parse it.
...
# --- 1) Write HTML into public/wordiness ---
$dstHtml = Join-Path $pubDir "wordiness-next-tiny-step-builder.html"
...
<title>Wordiness — Next Tiny Step Builder</title>
<style>
  :root{
...
<div class="wrap">
  <header>
    <div style="display:flex; gap:12px; align-items:flex-start;">
      ...
      <div>
        <h1>Wordiness — Next Tiny Step Builder</h1>
        <div class="sub">
  ...
  <section class="card">
    <div class="hd">
      <div style="max-width:820px;">
        ...
        <div class="tiny">Type a task, then generate step tiles. Click any tile to select it, then use “Make smaller”.</div>
      </div>
      <div class="row">
    ...
    <div class="bd">
      ...
      <div id="status" class="status">Tip: keep the first “Now” step tiny (30 seconds or less).</div>
        ...
        <div class="panel">
          <div class="row" style="justify-content:space-between;">
            <div>
              ...
              <div class="tiny">Tap a tile’s button to send it to Now/Next/Later. Tap the tile text to hear it.</div>
            </div>
            <div class="bankRow">
...
Write-Utf8NoBom $dstHtml $html
Write-Host "✅ Wrote $dstHtml"

# --- 2) Upsert manifest entry safely ---
$manifestPath = Join-Path $pubDir "manifest.json"
if (!(Test-Path $manifestPath)) { throw "manifest.json not found at $manifestPath" }

$bak = "$manifestPath.bak_{0}" -f (Get-Date -Format "yyyyMMdd_HHmmss")
Copy-Item $manifestPath $bak -Force
Write-Host "✅ Backup: $bak"

$raw = Get-Content $manifestPath -Raw -Encoding UTF8

# Quick structural fix: missing commas between adjacent objects at top-level
$raw = $raw -replace '}\s*{', "},`n{"

# Escape any accidental newlines inside JSON strings
$raw = Escape-NewlinesInsideJsonStrings $raw

# Normalize "description" -> "desc" if present (your manifest mostly uses "desc")
$raw = $raw -replace '"description"\s*:', '"desc":'

# Parse
try {
  $manifestObj = $raw | ConvertFrom-Json
} catch {
  Write-Host "❌ manifest.json still invalid JSON."
...
# Identify items array (manifest may be an array, or may contain an array)
$items = $null
$itemsProp = $null
...
$newItem = [pscustomobject]@{
  file     = "wordiness-next-tiny-step-builder.html"
  title    = "Next Tiny Step Builder"
  desc     = "Turn a big task into Now/Next/Later micro-steps. Includes Make Smaller + Focus mode."
  tags     = @("executive-function","planning","initiation","working-memory","dyslexia-friendly","tts")
  seedable = $false
  order    = 520
}
...
Write-Host ""
Write-Host "Next:"
Write-Host "  git add public/wordiness/wordiness-next-tiny-step-builder.html public/wordiness/manifest.json"
Write-Host "  git commit -m ""Add Next Tiny Step Builder wordiness game"""
Write-Host "  npm run dev  -> open /wordiness"
