...
<div class="wrap">
  <header>
    <div>
      <h1>${esc(title)}</h1>
      <div class="sub">Offline Social Thread export â€¢ step, highlight, karaoke, mystery words</div>
    </div>
...
(function(){
  ...
  function applyMystery(rawText, msgIndex, count){
    ...
    for (const it of chosenArr){
      ...
      const masked = "â–¯".repeat(Math.min(8, Math.max(4, real.length)));
  ...
  function render(){
    ...
    if (arr.length){
      arr.slice(0, 80).forEach(c => {
        ...
        div.innerHTML = '<div class="term">'+esc(term)+'</div><div class="def">'+esc(def || "â€”")+'</div>';
