# Polish-WordinessConfusables.ps1
...
$path = Join-Path $pub "wordiness-focus-confusables-pbdq.html"
if (!(Test-Path $pub)) { throw "Missing folder: $pub" }

# backup
$bakDir = Join-Path $PWD (".wordiness_backups\confusables_{0}" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
New-Item -ItemType Directory -Force -Path $bakDir | Out-Null
if (Test-Path $path) { Copy-Item $path (Join-Path $bakDir (Split-Path $path -Leaf)) -Force }
...
Write-Host "Next: npm run dev -> open /wordiness/wordiness-focus-confusables-pbdq.html"
